...
#Laso, Confusion Matrix, and ROC Curve
```{r, echo=TRUE}
set.seed(1)

#Prepare Data
election_data <- as.data.frame(cbind(scaled_facts,joined_facts_results$GOPDem))
colnames(election_data) <- c(colnames(election_data)[1:ncol(election_data)-1],"GOPDem")
# election_data$Winner[election_data$Winner == 0] = "Democrat"
# election_data$Winner[election_data$Winner == 1] = "GOP"

#Split into training and test sets and seperate (.5 to .5 train to test ratio)
election_data[,"train"] <- ifelse(runif(nrow(election_data))<0.5,1,0)
trainset <- election_data[election_data$train==1,]
testset <- election_data[election_data$train==0,]
...
#Fit model
glm_model <- glm(GOPDem~.,data = trainset, family = binomial)



#Predict probabilities on testset
typeColNum <- grep("GOPDem",names(election_data))
...
# contrasts(election_data$Winner)
...
#Confusion matrix
table(pred=glm_predict,true=testset$GOPDem)

#Accuracy
mean(glm_predict==testset$GOPDem)



########Laso#########

#Create X&Y Variables (for X, convert training data to matrix format)
x <- model.matrix(GOPDem~.,trainset)
y <- trainset$GOPDem
...
#Get test data
x_test <- model.matrix(GOPDem~.,testset)
...
#Confusion matrix
table(pred=lasso_predict,true=testset$GOPDem)

#Accuracy
mean(lasso_predict==testset$GOPDem)


######### ROC ##########
#Load probabilities into testset data
testset$prob = glm_prob

#Plot ROC Curve
plot.roc(GOPDem ~ prob, data = testset,smooth=TRUE,add=FALSE, reuse.auc=TRUE,
